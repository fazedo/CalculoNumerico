...
def f(t,u):
	return -50*(u-np.cos(t)) # Harier 1.1
	return np.array (-0.04*u[0] + 1e4*u[1]*u[2], 0.04*u[0] - 1e4*u[1]*u[2] - 3e7*u[2]**2, 3e7*u[2]**2  ) #Harier 1.4
...
def pred_corr_adams_45(h,Tmax,u1):
	dim=np.size(u1)
	itmax=np.int(Tmax/h)
	u=np.empty((itmax+1,dim))
	u[0,:]=u1
	...
	for i in np.arange(0,itmax-3):
		...
		for k in [0]:	
			fn[4] = f( (i+4)*h,  u[i+4,:])
			u[i+4,:] = u[i+3,:] + h*(-19*fn[0] + 106*fn[1] - 264*fn[2] + 646*fn[3]+ 251*fn[4])/720
...
for metodo in [pred_corr_adams_45, pred_corr_adams_4, adams_bash_4,  RK4_classico,adams_bash_2, pred_corr_adams_2, euler, euler_mod]:
	u=metodo(h,Tmax,u0)
	
	print u[itmax,:], metodo.__name__
